...
for (const q of QUIZ_QUESTIONS) {
  allSleep[q.id] = "sleep-opt";
  allStagnation[q.id] = "stagnation-opt";
}

describe("scoreQuiz", () => {
  it("every question offers a sleep-weighted and a stagnation-weighted option (test fixtures valid)", () => {
    expect(QUIZ_QUESTIONS.every(q => q.options.some(o => o.id === "sleep-opt") && q.options.some(o => o.id === "stagnation-opt"))).toBe(true);
  });
  ...
  it("accumulates weights across questions and reports per-pattern scores", () => {
    ...
    expect(r.scores.sleep).toBeGreaterThan(r.scores.overstim);
    expect(Object.keys(r.scores)).toEqual(["sleep", "overload", "overstim", "stagnation", "rhythm"]);
  });
  it("breaks ties by fixed order sleep > overload > overstim > stagnation > rhythm", () => {
    const empty: Record<string, string> = {};
    for (const q of QUIZ_QUESTIONS) empty[q.id] = "neutral-opt";
...
- [ ] **Step 2: Verify in browser** — dev server at `/start`: click through selecting every first option; assert result card shows `PATTERN_INFO` name for the expected pattern (first options favor `sleep` per `content/quiz.ts`: q1 sleep-opt, q2 overload-opt, q3 neutral, q4 neutral, q5 overstim, q6 neutral, q7 overload-opt → scores sleep 2, overload 4 → primary "overload"); restart returns to intro; progress shows "7 of 7" before result.
