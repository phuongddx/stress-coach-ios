...
- **Release:** `deploy.yml` was removed. TestFlight upload is `.github/workflows/testflight.yml` (GitHub Actions, `asc` CLI + raw p12 cert `XPT2DHR688`, not Fastlane/Match) — despite README's "How it ships" table still describing Xcode Cloud PR/Beta/Release workflows as the pipeline. Xcode Cloud today only provisions Firebase config (`StressMonitor/ci_scripts/ci_post_clone.sh`); its Beta workflow's Xcode/macOS image pairing went invalid, so a weekly `.github/workflows/xcodecloud-image-guard.yml` job checks whether it's safe to re-enable. Remaining manual workflows: `distribute.yml` (`fastlane distribute_beta`), `release.yml` (`fastlane release`; `build_number`/`submit_for_review` inputs are ignored by the lane), `match.yml`. `testflight.yml` requires repo secrets `GOOGLE_SERVICE_INFO_PLIST_BASE64`, `BUILD_CERTIFICATE_P12_BASE64`, `P12_PASSWORD` (all set 2026-09-23/24); its archive gate runs BEFORE Export IPA so a verify failure never uploads the signed-IPA artifact on this public repo. `main` has NO required status checks — the PR gate is advisory until branch protection is configured.
